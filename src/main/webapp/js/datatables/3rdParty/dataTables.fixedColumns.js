	...
	"_fnConstruct": function ( oInit )
	{
		...
		jqTable
			...
			.on( 'select.dt.DTFC deselect.dt.DTFC', function ( e, dt, type, indexes ) {
				if (e.namespace !== "dt") {//MODIFIED FOR EPL - check is added to prevent redraw fc when native 'select' is triggered by browser when focusing 'numeric.js' inputs
					return false;
				}
				that._fnDraw( false );
			} )
			.on( 'destroy.dt.DTFC', function () {
				...
				$(window).off( '.DTFC' );
				$(that.s.dt.nTableWrapper).off( '.DTFC' );//this -> that; MODIFIED FOR EPL
